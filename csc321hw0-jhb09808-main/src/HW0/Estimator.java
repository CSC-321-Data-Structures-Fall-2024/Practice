...
public class Estimator
{
     ...
     * each bus has.
     */
    public static long estimateNumBusses(int hours, int seats)
    {
        Bus bus = new Bus(seats);
        int miles = hours * 60; // assume 60 mph
        int feetPerMile = 5280;
        long totalFeet = feetPerMile * miles;
        long numBusses = totalFeet / bus.getLength();
        return numBusses;
    }
    ...
    public static double freeVolume(int volume, int percentOccupied)
    {
        ...
        freeVolume = volume * (percentFree / 100.0); // Use 100.0 to ensure floating-point division
        return freeVolume;
    }
}
